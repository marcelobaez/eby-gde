...
export default function ToldoPage() {
  ...
  const { data: usersByLocation } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get<UsersByLocationResponse>(
        "/api/toldo/users"
      );
      return data;
    },
      ...
      <Row gutter={[16, 16]} justify="center">
        <Card style={{ minHeight: "calc(100dvh - 210px)", width: "100%" }}>
          <Space direction="vertical" size="middle" style={{ display: "flex" }}>
            ...
            {data &&
              ...
              (() => {
                ...
                const bgGradients: Record<string, string> = {
                  "#52c41a": "linear-gradient(to right, #f6ffed, #ffffff)",
                  "#1890ff": "linear-gradient(to right, #e6f4ff, #ffffff)",
                  "#fa8c16": "linear-gradient(to right, #fff7e6, #ffffff)",
                  "#f5222d": "linear-gradient(to right, #fff1f0, #ffffff)",
                };
                return (
                  <Row gutter={[16, 16]}>
                    <Col span={6}>
            ...
            {data && data.stats && data.stats.docTypeStats.length > 0 && (
              <Row gutter={[4, 4]}>
                {data.stats.docTypeStats.map((stat, index) => {
                  ...
                  const selected = isTypeSelected(stat.tipo_documento);
                  return (
                    <Col span={4} key={stat.tipo_documento || index}>
                      <Card
                        size="small"
                        hoverable
                        onClick={() => toggleDocTypeFilter(stat.tipo_documento)}
                        style={{
                          ...
                          description={
                            <Space direction="vertical" size={2}>
                              ...
                              <Text
                                ...
                                style={{ fontSize: "12px" }}
                              >
                                Generado por: {item.datos_usuario} - Fecha:{" "}
