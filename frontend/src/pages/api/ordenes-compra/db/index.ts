...
export default async function handler(
  ...
  try {
    ...
    const data: OrdenCompraDBResult[] = result.rows.map(row => ({
      oc: row.oc?.trim(),
      id_empresa: row.id_empresa?.trim(),
      descripcion: row.descripcion?.trim(),
      dtipodoc: row.dtipodoc?.trim(),
      dmodalidadcompra: row.dmodalidadcompra?.trim(),
      nnumeroconcurso: row.nnumeroconcurso?.trim(),
      dsede: row.dsede?.trim(),
      destado: row.destado?.trim(),
      fecha_estado: row.fecha_estado?.trim(),
      dnombre_empresa: row.dnombre_empresa?.trim(),
      dsdc: row.dsdc?.trim(),
      nexpediente: row.nexpediente?.trim(),
      ffechaemision: row.ffechaemision?.trim(),
      ncosto: row.ncosto?.trim(),
      dmoneda: row.dmoneda?.trim(),
      ncostobase: row.ncostobase?.trim(),
      ffechaenvioprov: row.ffechaenvioprov?.trim(),
      despecialidad: row.despecialidad?.trim(),
    }));
